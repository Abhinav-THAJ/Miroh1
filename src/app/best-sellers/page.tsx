...
export default function BestSellersPage() {
  ...
  const [products, setProducts] = useState<any[]>(ALL_PRODUCTS.filter((p) => p.isBestSeller));
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      if (wcCategories && wcCategories.length > 0) {
        setCategories([
          { id: "all", name: "All", slug: "all", count: ALL_PRODUCTS.filter(p => p.isBestSeller).length },
          ...wcCategories
      ...
      if (wcProducts && wcProducts.length > 0) {
        ...
        const formattedProducts = wcProducts.map((p: any) => {
          ...
          return {
            ...
            rating: 4.9,
            reviewCount: 42,
            shortDescription: p.short_description ? p.short_description.replace(/<[^>]*>?/gm, "") : "Most Loved Piece",
            description: p.description ? p.description.replace(/<[^>]*>?/gm, "") : "Bestselling luxury design",
            ...
            features: ["Most loved design", "100% anti-tarnish"],
            inStock: true,
          };
        });
        ...
        <div className="container mx-auto px-6">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h1 className="text-4xl sm:text-6xl font-serif text-warm-ivory mb-6">
              Best <span className="italic text-champagne-gold font-light">Sellers</span>
            </h1>
            <p className="text-muted-text font-light text-base sm:text-lg leading-relaxed">
              Explore the iconic designs chosen most by our clientele. Timeless statement pieces crafted for perfection.
            </p>
          </div>

          <Suspense fallback={<div className="py-20 text-center text-warm-ivory">Loading best sellers...</div>}>
